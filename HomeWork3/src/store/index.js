...
export default new Vuex.Store({
  ...
  getters: {
    postList: (state) => {
      let postList = state.postList.map((post) => {
        return {
          userId: post.userId,
  ...
  mutations: {
    //ADD LIKE TO POST
    addLike: (state, data) => {
      state.postList[data.userId - 1].likes++
    },
    //RESET ALL POST LIKES
    resetLikes: (state) => {
  ...
  actions: {
    //ACTIONS FOR USE IN OTHER COMPONENTS/VIEWS
    //USED IN POSTS.VUE TO ADD LIKE TO 1 POST
    increaseLikes: (act, post) => {
      console.log("Like added to post")
      setTimeout(() => {act.commit("addLike", post)}, 0)},
    //USED IN HOME.VUE TO RESET LIKES
    resetLikes: (act) => {
      console.log("Likes reset")
      setTimeout(() => {act.commit("resetLikes")}, 500)}
  }
})
